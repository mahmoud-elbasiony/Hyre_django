...
class InterviewDetailView(generics.GenericAPIView):
    ...
    def get(self, request, pk):
        ...
        if interview == None:
            return Response({"status": False, "message": f"Interview with Id: {pk} not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def delete(self, request, pk):
        interview = self.get_Interview(pk)
        if interview == None:
            return Response({"success": False, "message": f"Interview with Id: {pk} not found"}, status=status.HTTP_404_NOT_FOUND)
